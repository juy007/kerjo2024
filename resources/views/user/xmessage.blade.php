                            ...
                            <div class="chat-message-list" data-simplebar style="max-height: 900px;">
                                <ul class="list-unstyled chat-list">
                                    ...
                                    <li class="{{ $latestMessage['status'] }}">
                                        <a href="#" onclick="loadUserMessages('{{ $latestMessage['_id'] }}')">
                                            <div class="d-flex align-items-start">
                                                <!-- Avatar Pengirim -->
                                                <div class="flex-shrink-0 user-img online align-self-center me-3">
                                                    <img src="{{ url('proxy-image/avatar/'. str_replace('../public/upload/avatar/', '', $latestMessage['sender_avatar'] )) }}" class="rounded-circle avatar-sm" alt="{{ $latestMessage['sender_name'] }}">
...
<script>
    function loadUserMessages(userId) {
        // Menampilkan loading atau gambar default saat menunggu data
        document.getElementById('zonaChat').innerHTML = '<div class="d-flex justify-content-center align-items-center" style="height: 80vh;">' +
            '<img src="' + '{{ url('proxy-image/src/loadchat.gif') }}' + '" class="img-fluid w-5" alt="Loading">' +
            '</div>';


        // AJAX atau fetch untuk mendapatkan pesan berdasarkan userId
        fetch("{{ route('detail_message', ['id' => '__userId__']) }}".replace('__userId__', userId))
            .then(response => response.json())
            .then(data => {
                // Update zona chat langsung dengan HTML yang dikirimkan
                document.getElementById('zonaChat').innerHTML = data.userHtml;/* + data.messageHtml;*/
            })
            .catch(error => {
